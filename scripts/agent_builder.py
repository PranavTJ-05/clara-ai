...
def build_agent_spec(memo: AccountMemo) -> RetellAgentSpec:
    ...
    # Construct business hours flow
    business_hours_flow = f"""
    === BUSINESS HOURS FLOW ===
    ...
    4. Determine routing based on the following rules:
       - Non-emergencies: {memo.non_emergency_routing_rules or 'Leave a message.'}
       - Emergencies: Transfer immediately based on rules.
    5. Attempt transfer: {memo.call_transfer_rules or 'Transfer to main office.'}
    ...
    3. Determine if emergency based on definition:
       {', '.join(memo.emergency_definition) if memo.emergency_definition else 'Use best judgment based on safety.'}
    4. If EMERGENCY:
    ...
    5. If NON-EMERGENCY:
       ...
       - Promise follow-up during standard business hours ({memo.business_hours or 'unknown hours'}).
    ...
    SPECIAL CONSTRAINTS:
    {memo.integration_constraints or 'None'}
    
    UNKNOWN DETAILS TO AVOID INVENTING:
    {memo.questions_or_unknowns or 'None'}
    """
